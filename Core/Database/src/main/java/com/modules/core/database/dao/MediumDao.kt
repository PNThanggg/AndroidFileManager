package com.modules.core.database.dao
...
interface MediumDao {
    ...
    @Query(
        "UPDATE OR REPLACE media SET " + "external_subs = :externalSubs, " + "video_scale = :videoScale " + "WHERE uri = :uri",
    )
    ...
    @Query(
        "UPDATE OR REPLACE media SET " + "playback_position = :position, " + "audio_track_index = :audioTrackIndex, " + "subtitle_track_index = :subtitleTrackIndex, " + "playback_speed = :playbackSpeed, " + "last_played_time = :lastPlayedTime " + "WHERE uri = :uri",
    )
