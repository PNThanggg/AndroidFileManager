...
class RecyclerViewFastScroller @JvmOverloads constructor(
     ...
     * */
    private var handleVisibilityDuration: Int = -1
                    ...
                    MotionEvent.ACTION_MOVE, MotionEvent.ACTION_DOWN -> {
                        ...
                        if (isFastScrollEnabled) {
                            ...
                            when ((recyclerView.layoutManager as LinearLayoutManager).orientation) {
                                RecyclerView.HORIZONTAL -> recyclerView.scrollBy(
                                    currentRelativePos.toInt(), 0
                                )

                                RecyclerView.VERTICAL -> recyclerView.scrollBy(
                                    0, currentRelativePos.toInt()
                                )
                            }
                        }
    ...
    private fun moveViewToRelativePositionWithBounds(view: View, finalOffset: Float) {
        when (fastScrollDirection) {
            FastScrollDirection.HORIZONTAL -> view.x =
                min(max(finalOffset, 0F), (trackLength - view.width.toFloat()))

            FastScrollDirection.VERTICAL -> view.y =
                min(max(finalOffset, 0F), (trackLength - view.height.toFloat()))
        }
    }
