...
public class IconNormalizer {
    ...
    private static final float LINEAR_SCALE_SLOPE = (MAX_CIRCLE_AREA_FACTOR - MAX_SQUARE_AREA_FACTOR) / (1 - CIRCLE_AREA_BY_RECT);
    ...
    private final boolean mEnableShapeDetection;
    ...
    public static float normalizeAdaptiveIcon(Drawable d, int size, @Nullable RectF outBounds) {
        ...
        if (outBounds != null) {
            outBounds.set(hullBounds.left / (float) size, hullBounds.top / (float) size, 1 - (hullBounds.right / (float) size), 1 - (hullBounds.bottom / (float) size));
        }
