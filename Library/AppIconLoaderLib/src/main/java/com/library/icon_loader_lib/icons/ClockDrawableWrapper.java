...
import static com.library.icon_loader_lib.icons.ThemedIconDrawable.getColors;
...
public class ClockDrawableWrapper extends AdaptiveIconDrawable implements BitmapInfo.Extender {
    ...
    private static final String ROUND_ICON_METADATA_KEY = LAUNCHER_PACKAGE + ".LEVEL_PER_TICK_ICON_ROUND";
    ...
    private static final String DEFAULT_HOUR_METADATA_KEY = LAUNCHER_PACKAGE + ".DEFAULT_HOUR";
    private static final String DEFAULT_MINUTE_METADATA_KEY = LAUNCHER_PACKAGE + ".DEFAULT_MINUTE";
    private static final String DEFAULT_SECOND_METADATA_KEY = LAUNCHER_PACKAGE + ".DEFAULT_SECOND";
    ...
    public static ClockDrawableWrapper forPackage(Context context, String pkg, int iconDpi) {
        try {
            ...
            ApplicationInfo appInfo = pm.getApplicationInfo(pkg, PackageManager.MATCH_UNINSTALLED_PACKAGES | PackageManager.GET_META_DATA);
            Resources res = pm.getResourcesForApplication(appInfo);
            return forExtras(appInfo, appInfo.metaData, resId -> res.getDrawableForDensity(resId, iconDpi));
        } catch (Exception e) {
    ...
    private static ClockDrawableWrapper fromThemeData(Context context, ThemedIconDrawable.ThemeData themeData) {
        try {
            ...
            for (int i = 0; i < count; i += 2) {
                ...
                extras.putInt(ta.getString(i), v.type >= TypedValue.TYPE_FIRST_INT && v.type <= TypedValue.TYPE_LAST_INT ? v.data : v.resourceId);
            }
            ta.recycle();
            ClockDrawableWrapper drawable = ClockDrawableWrapper.forExtras(context.getApplicationInfo(), extras, resId -> {
    ...
    private static ClockDrawableWrapper forExtras(ApplicationInfo appInfo, Bundle metadata, IntFunction<Drawable> drawableProvider) {
        if (metadata == null) {
            return null;
        }
        ...
        ClockDrawableWrapper wrapper = new ClockDrawableWrapper((AdaptiveIconDrawable) drawable);
    ...
    @Override
    public ClockBitmapInfo getExtendedInfo(Bitmap bitmap, int color, BaseIconFactory iconFactory, float normalizationScale, UserHandle user) {
        iconFactory.disableColorExtraction();
        AdaptiveIconDrawable background = new AdaptiveIconDrawable(getBackground().getConstantState().newDrawable(), null);
        BitmapInfo bitmapInfo = iconFactory.createBadgedIconBitmap(background, Process.myUserHandle(), mTargetSdkVersion, false);

        return new ClockBitmapInfo(bitmap, color, normalizationScale, mAnimationInfo, bitmapInfo.icon, mThemeData);
    }
    ...
    static class ClockBitmapInfo extends BitmapInfo {
        ...
        ClockBitmapInfo(Bitmap icon, int color, float scale, AnimationInfo animInfo, Bitmap background, ThemedIconDrawable.ThemeData themeData) {
            this(icon, color, scale, animInfo, background, themeData, null);
        }

        ClockBitmapInfo(Bitmap icon, int color, float scale, AnimationInfo animInfo, Bitmap background, ThemedIconDrawable.ThemeData themeData, ColorFilter bgFilter) {
            super(icon, color);
            this.scale = scale;
        ...
        public FastBitmapDrawable newThemedIcon(Context context) {
            if (themeData != null) {
                ...
                if (wrapper != null) {
                    ...
                    return new ClockBitmapInfo(icon, colors[1], scale, wrapper.mAnimationInfo, mFlattenedBackground, themeData, bgFilter).newIcon(context);
                }
            }
    ...
    private static class ClockIconDrawable extends FastBitmapDrawable implements Runnable {
        ...
        ClockIconDrawable(ClockBitmapInfo clockInfo) {
            ...
            mInfo = clockInfo;
            mFullDrawable = (AdaptiveIconDrawable) mInfo.animInfo.baseDrawableState.newDrawable().mutate();
        ...
        public void drawInternal(Canvas canvas, Rect bounds) {
            ...
            canvas.scale(mInfo.scale, mInfo.scale, bounds.exactCenterX() + mInfo.offset, bounds.exactCenterY() + mInfo.offset);
