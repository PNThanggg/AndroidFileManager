...
import static com.library.icon_loader_lib.icons.ShadowGenerator.BLUR_FACTOR;
...
public class BaseIconFactory implements AutoCloseable {
    ...
    private static final int PLACEHOLDER_BACKGROUND_COLOR = Color.rgb(245, 245, 245);
    ...
    protected BaseIconFactory(Context context, int fillResIconDpi, int iconBitmapSize, boolean shapeDetection) {
        ...
        mTextPaint.setTextSize(context.getResources().getDisplayMetrics().density * PLACEHOLDER_TEXT_SIZE);
        clear();
    }
    ...
    public BitmapInfo createIconBitmap(Intent.ShortcutIconResource iconRes) {
        try {
            ...
            return createBadgedIconBitmap(resources.getDrawableForDensity(id, mFillResIconDpi), Process.myUserHandle() /* only available on primary user */, false /* do not apply legacy treatment */);
        } catch (Exception e) {
            // Icon not found.
        }
        return null;
    ...
    public BitmapInfo createIconBitmap(String placeholder, int color) {
        ...
        Bitmap placeholderBitmap = Bitmap.createBitmap(mIconBitmapSize, mIconBitmapSize, Bitmap.Config.ARGB_8888);
        mTextPaint.setColor(color);
        Canvas canvas = new Canvas(placeholderBitmap);
        canvas.drawText(placeholder, (float) mIconBitmapSize / 2, (float) (mIconBitmapSize * 5) / 8, mTextPaint);
        AdaptiveIconDrawable drawable = new AdaptiveIconDrawable(new ColorDrawable(PLACEHOLDER_BACKGROUND_COLOR), new BitmapDrawable(mContext.getResources(), placeholderBitmap));
    ...
    public BitmapInfo createShapedIconBitmap(Bitmap icon, UserHandle user) {
        ...
        if (ATLEAST_OREO) {
            ...
            d = new AdaptiveIconDrawable(new ColorDrawable(Color.BLACK), new InsetDrawable(d, inset, inset, inset, inset));
        }
    ...
    public BitmapInfo createBadgedIconBitmap(Drawable icon, UserHandle user, int iconAppTargetSdk, boolean isInstantApp, float[] scale) {
        boolean shrinkNonAdaptiveIcons = ATLEAST_P || (ATLEAST_OREO && iconAppTargetSdk >= Build.VERSION_CODES.O);
    ...
    public Bitmap getUserBadgeBitmap(UserHandle user) {
        if (mUserBadgeBitmap == null) {
            Bitmap bitmap = Bitmap.createBitmap(mIconBitmapSize, mIconBitmapSize, Bitmap.Config.ARGB_8888);
            Drawable badgedDrawable = mPm.getUserBadgedIcon(new FixedSizeBitmapDrawable(bitmap), user);
            if (badgedDrawable instanceof BitmapDrawable) {
                ...
                mUserBadgeBitmap = BitmapRenderer.createSoftwareBitmap(mIconBitmapSize, mIconBitmapSize, badgedDrawable::draw);
            }
        }
    ...
    public Bitmap createScaledBitmapWithoutShadow(Drawable icon, boolean shrinkNonAdaptiveIcons) {
        ...
        return createIconBitmap(icon, Math.min(scale[0], ShadowGenerator.getScaleForBounds(iconBounds)));
    }
    ...
    public void badgeWithDrawable(Canvas target, Drawable badge) {
        ...
        if (mBadgeOnLeft) {
            ...
            badge.setBounds(mIconBitmapSize - badgeSize, mIconBitmapSize - badgeSize, mIconBitmapSize, mIconBitmapSize);
        }
        badge.draw(target);
    }
    ...
    public Bitmap createIconBitmap(@NonNull Drawable icon, float scale, int size) {
        ...
        if (ATLEAST_OREO && icon instanceof AdaptiveIconDrawable) {
            ...
            if (icon instanceof BitmapInfo.Extender) {
                ((BitmapInfo.Extender) icon).drawForPersistence(mCanvas);
            } else {
                icon.draw(mCanvas);
            }
        } else {
            if (icon instanceof BitmapDrawable) {
                ...
                if (b.getDensity() == Bitmap.DENSITY_NONE) {
            ...
            mCanvas.save();
            mCanvas.scale(scale, scale, (float) size / 2, (float) size / 2);
            icon.draw(mCanvas);
            mCanvas.restore();
    ...
    public BitmapInfo makeDefaultIcon(UserHandle user) {
        return createBadgedIconBitmap(getFullResDefaultActivityIcon(mFillResIconDpi), user, Build.VERSION.SDK_INT);
    }

    public static Drawable getFullResDefaultActivityIcon(int iconDpi) {
        return Resources.getSystem().getDrawableForDensity(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O ? android.R.drawable.sym_def_app_icon : android.R.mipmap.sym_def_app_icon, iconDpi);
    }
