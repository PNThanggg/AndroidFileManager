...
public class IconCacheUpdateHandler {
    ...
    private <T> void updateIconsPerUser(UserHandle user, HashMap<ComponentName, T> componentMap, CachingLogic<T> cachingLogic, OnUpdateCallback onUpdateCallback) {
        ...
        try (Cursor c = mIconCache.mIconDb.query(new String[]{BaseIconCache.IconDB.COLUMN_ROWID, BaseIconCache.IconDB.COLUMN_COMPONENT, BaseIconCache.IconDB.COLUMN_LAST_UPDATED, BaseIconCache.IconDB.COLUMN_VERSION, BaseIconCache.IconDB.COLUMN_SYSTEM_STATE}, BaseIconCache.IconDB.COLUMN_USER + " = ? ", new String[]{Long.toString(userSerial)})) {

            final int indexComponent = c.getColumnIndex(BaseIconCache.IconDB.COLUMN_COMPONENT);
            final int indexLastUpdate = c.getColumnIndex(BaseIconCache.IconDB.COLUMN_LAST_UPDATED);
            final int indexVersion = c.getColumnIndex(BaseIconCache.IconDB.COLUMN_VERSION);
            final int rowIndex = c.getColumnIndex(BaseIconCache.IconDB.COLUMN_ROWID);
            final int systemStateIndex = c.getColumnIndex(BaseIconCache.IconDB.COLUMN_SYSTEM_STATE);
    ...
    public void finish() {
        // Commit all deletes
        int deleteCount = 0;
        StringBuilder queryBuilder = new StringBuilder().append(BaseIconCache.IconDB.COLUMN_ROWID).append(" IN (");
