...
import com.library.icon_loader_lib.util.SafeCloseable;
...
public class IconProvider {
    ...
    public Drawable getIcon(LauncherActivityInfo info, int iconDpi) {
        return getIconWithOverrides(info.getApplicationInfo().packageName, info.getUser(), iconDpi, () -> info.getIcon(iconDpi));
    }
    ...
    public Drawable getIcon(ActivityInfo info, int iconDpi) {
        return getIconWithOverrides(info.applicationInfo.packageName, UserHandle.getUserHandleForUid(info.applicationInfo.uid), iconDpi, () -> loadActivityInfoIcon(info, iconDpi));
    }

    private Drawable getIconWithOverrides(String packageName, UserHandle user, int iconDpi, Supplier<Drawable> fallback) {
        Drawable icon = null;
        ...
        } else if (mClock != null && mClock.getPackageName().equals(packageName) && Process.myUserHandle().equals(user)) {
        ...
        ThemedIconDrawable.ThemeData td = getThemedIconMap().get(packageName);
    ...
    private Drawable loadActivityInfoIcon(ActivityInfo ai, int density) {
        ...
        if (density != 0 && iconRes != 0) {
            try {
                final Resources resources = mContext.getPackageManager().getResourcesForApplication(ai.applicationInfo);
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth) && type != XmlPullParser.END_DOCUMENT) {
                    ...
                    if (TAG_ICON.equals(parser.getName())) {
                        ...
                        if (iconId != 0 && !TextUtils.isEmpty(pkg)) {
                            map.put(pkg, new ThemedIconDrawable.ThemeData(res, iconId));
                        }
                    }
                }
            }
    ...
    public String getSystemIconState() {
        return (CONFIG_ICON_MASK_RES_ID == ID_NULL ? "" : mContext.getResources().getString(CONFIG_ICON_MASK_RES_ID)) + (mThemedIconMap == DISABLED_MAP ? ",no-theme" : ",with-theme");
    }
