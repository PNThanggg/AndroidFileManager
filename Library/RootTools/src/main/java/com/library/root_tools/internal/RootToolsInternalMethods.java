...
public class RootToolsInternalMethods {
    ...
    public Permissions getPermissions(String line) {
        ...
        if (rawPermissions.length() == 10 && (rawPermissions.charAt(0) == '-' || rawPermissions.charAt(0) == 'd' || rawPermissions.charAt(0) == 'l') && (rawPermissions.charAt(1) == '-' || rawPermissions.charAt(1) == 'r') && (rawPermissions.charAt(2) == '-' || rawPermissions.charAt(2) == 'w')) {
            ...
            String finalPermissions = String.valueOf(parseSpecialPermissions(rawPermissions)) + parsePermissions(permissions.getUserPermissions()) + parsePermissions(permissions.getGroupPermissions()) + parsePermissions(permissions.getOtherPermissions());
    ...
    public List<String> getBusyBoxApplets(String path) throws Exception {
        ...
        Command command = new Command(Constants.BBA, false, path + "busybox --list") {
            ...
            public void commandOutput(int id, String line) {
                if (id == Constants.BBA) {
                    if (!line.trim().isEmpty() && !line.trim().contains("not found") && !line.trim().contains("file busy")) {
                        results.add(line);
                    }
                }
            ...
            Command command = new Command(Constants.BBV, false, path + "busybox") {
                ...
                public void commandOutput(int id, String line) {
                    ...
                    if (id == Constants.BBV) {
                        ...
                        if (temp.length > 1 && temp[1].contains("v1.")) {
    ...
    public String getInode(String file) {
        try {
            Command command = new Command(Constants.GI, false, "/data/local/ls -i " + file) {
                ...
                public void commandOutput(int id, String line) {
                    if (id == Constants.GI) {
                        if (!line.trim().isEmpty() && Character.isDigit(line.trim().substring(0, 1).toCharArray()[0])) {
    ...
    public Permissions getFilePermissionsSymlinks(String file) {
        ...
        if (RootTools.exists(file)) {
            ...
            try {
                ...
                Command command = new Command(Constants.FPS, false, "ls -l " + file, "busybox ls -l " + file, "/system/bin/failsafe/toolbox ls -l " + file, "toolbox ls -l " + file) {
                    @Override
                    public void commandOutput(int id, String line) {
                        if (id == Constants.FPS) {
    ...
    public ArrayList<Mount> getMounts() throws Exception {
        ...
        Command cmd = new Command(Constants.GET_MOUNTS, false, "cat /proc/mounts") {
    ...
    public String getSymlink(String file) {
        ...
        try {
            ...
            if (symlink.length > 2 && symlink[symlink.length - 2].equals("->")) {
                ...
                if (!symlink[symlink.length - 1].isEmpty() && !symlink[symlink.length - 1].contains("/")) {
    ...
    public void offerSuperUser(Activity activity) {
        ...
        Intent i = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=eu.chainfire.supersu"));
        activity.startActivity(i);
    }
