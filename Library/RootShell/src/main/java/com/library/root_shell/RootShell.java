package com.library.root_shell;
...
public class RootShell {
    ...
    public static final String version = "RootShell";
    ...
    public static List<String> findBinary(final String binaryName, List<String> searchPaths, boolean singlePath) {
        ...
        try {
            for (String path : searchPaths) {
                ...
                Command cc = getCommand(binaryName, path, foundPaths);

                cc = RootShell.getShell(false).add(cc);
                commandWait(RootShell.getShell(false), cc);

                if (!foundPaths.isEmpty() && singlePath) {
                    break;
                }
            }
    ...
    @NonNull
    private static Command getCommand(String binaryName, String path, List<String> foundPaths) {
        final String currentPath = path;

        return new Command(0, false, "stat " + path + binaryName) {
            @Override
            public void commandOutput(int id, String line) {
    ...
    public static boolean isBusyboxAvailable(boolean includeToybox) {
        if (includeToybox) {
            return !(findBinary("busybox", true)).isEmpty() || !(findBinary("toybox", true)).isEmpty();
        } else {
            return !(findBinary("busybox", true)).isEmpty();
        }
    }
    ...
    public static boolean isRootAvailable() {
        return !(findBinary("su", true)).isEmpty();
    }
