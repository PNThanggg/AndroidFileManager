...
import com.modules.feature.player.extensions.getName
        ...
        when (type) {
            C.TRACK_TYPE_AUDIO -> {
                return activity?.let { activity ->
                    val audioTracks =
                        tracks.groups.filter { it.type == C.TRACK_TYPE_AUDIO && it.isSupported }
                    ...
                    val selectedTrackIndex =
                        audioTracks.indexOfFirst { it.isSelected }.takeIf { it != -1 }
                            ?: audioTracks.size
            ...
            C.TRACK_TYPE_TEXT -> {
                return activity?.let { activity ->
                    val textTracks =
                        tracks.groups.filter { it.type == C.TRACK_TYPE_TEXT && it.isSupported }
                    ...
                    val selectedTrackIndex =
                        textTracks.indexOfFirst { it.isSelected }.takeIf { it != -1 }
                            ?: textTracks.size
