package com.modules.feature.player.service
...
import com.module.core.extensions.deleteFiles
import com.module.core.extensions.getFilenameFromUri
...
import com.modules.feature.player.extensions.switchTrack
...
class PlayerService : MediaSessionService() {
    ...
    private val playbackStateListener = object : Player.Listener {
                ...
                -> {
                    ...
                    if (newMediaItem != null && oldMediaItem != newMediaItem) {
                        mediaRepository.updateMediumPosition(
                            ...
                            position = oldPosition.positionMs.takeIf { reason == DISCONTINUITY_REASON_SEEK }
                                ?: C.TIME_UNSET,
                        )
                    }
                }
        ...
        override fun onPlaybackStateChanged(playbackState: Int) {
            ...
            if (playbackState == Player.STATE_ENDED || playbackState == Player.STATE_IDLE) {
                mediaSession?.player?.trackSelectionParameters =
                    TrackSelectionParameters.getDefaults(this@PlayerService)
    ...
    private val mediaSessionCallback = object : MediaSession.Callback {
        override fun onConnect(
            ...
            return MediaSession.ConnectionResult.accept(
                connectionResult.availableSessionCommands.buildUpon()
                    .addSessionCommands(customCommands).build(),
        ...
        override fun onSetMediaItems(
            ...
            serviceScope.future(Dispatchers.Default) {
                val (updatedMediaItems, _) = measureTimedValue {
                    updatedMediaItemsWithMetadata(mediaItems)
                }
                return@future MediaSession.MediaItemsWithStartPosition(
                    updatedMediaItems, startIndex, startPositionMs
                )
            }
        ...
        override fun onCustomCommand(
            ...
            val command =
                CustomCommands.fromSessionCommand(customCommand) ?: return@future SessionResult(
            ...
            when (command) {
                CustomCommands.ADD_SUBTITLE_TRACK -> {
                    val subtitleUri =
                        args.getString(CustomCommands.SUBTITLE_TRACK_URI_KEY)?.let { Uri.parse(it) }
                ...
                CustomCommands.GET_AUDIO_SESSION_ID -> {
                    val audioSessionId =
                        mediaSession?.player?.audioSessionId ?: C.AUDIO_SESSION_ID_UNSET
                    return@future SessionResult(
                        SessionResult.RESULT_SUCCESS,
                        Bundle().apply {
    ...
    override fun onCreate() {
        ...
        val trackSelector = DefaultTrackSelector(applicationContext).apply {
            setParameters(
                buildUponParameters().setPreferredAudioLanguage(playerPreferences.preferredAudioLanguage)
                    .setPreferredTextLanguage(playerPreferences.preferredSubtitleLanguage),
            )
        }

        val player = ExoPlayer.Builder(applicationContext).setRenderersFactory(renderersFactory)
            .setTrackSelector(trackSelector).setAudioAttributes(
                AudioAttributes.Builder().setUsage(C.USAGE_MEDIA)
                    .setContentType(C.AUDIO_CONTENT_TYPE_MOVIE).build(),
                playerPreferences.requireAudioFocus,
            ).setHandleAudioBecomingNoisy(playerPreferences.pauseOnHeadsetDisconnect).build().also {
        ...
        try {
            mediaSession = MediaSession.Builder(this, player).apply {
                ...
                setCustomLayout(
                    listOf(
                        CommandButton.Builder().setIconResId(R.drawable.ic_close)
                            .setDisplayName(getString(R.string.stop_player_session))
    ...
    private suspend fun updatedMediaItemsWithMetadata(
        ...
        mediaItems.map { mediaItem ->
            async {
                ...
                val title =
                    mediaItem.mediaMetadata.title ?: mediaState?.title ?: getFilenameFromUri(uri)
                val artwork =
                    mediaState?.thumbnailPath?.let { Uri.parse(it) } ?: Uri.Builder().apply {
                ...
                val existingSubConfigurations =
                    mediaItem.localConfiguration?.subtitleConfigurations ?: emptyList()
...
private var Player.skipSilenceEnabled: Boolean
    @OptIn(UnstableApi::class) get() = when (this) {
