        ...
        object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onLongPress(e: MotionEvent) {
                ...
                activity.showTopInfo(
                    activity.getString(
                        R.string.fast_playback_speed, prefs.longPressControlsSpeed
                    )
                )
    ...
    private val seekGestureDetector = GestureDetector(
        ...
        object : GestureDetector.SimpleOnGestureListener() {
            override fun onScroll(
                ...
                val distanceDiff = abs(distanceX.pxToDp / 4).coerceIn(0.5f, 10f)
                val change = (distanceDiff * SEEK_STEP_MS).toLong()

                playerView.player?.run {
                    if (distanceX < 0L) {
                        seekChange = (seekChange + change).takeIf { it + seekStart < duration }
                            ?: (duration - seekStart)
                        position = (seekStart + seekChange).coerceAtMost(duration)
                        seekForward(positionMs = position, shouldFastSeek = shouldFastSeek)
                    } else {
                        seekChange =
                            (seekChange - change).takeIf { it + seekStart > 0 } ?: (0 - seekStart)
                        position = seekStart + seekChange
                        seekBack(positionMs = position, shouldFastSeek = shouldFastSeek)
                    }
                    activity.showPlayerInfo(
                        info = this.currentPosition.formatDurationMillis,
                        subInfo = "[${seekChange.formatDurationMillisSign}]",
                    )
                    return true
                }
                return false
            ...
            override fun onScroll(
                ...
                if (firstEvent.x.toInt() > viewCenterX) {
                    ...
                    volumeManager.setVolume(
                        volumeManager.currentVolume + change, prefs.showSystemVolumePanel
                    )
        ...
        object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            override fun onScale(detector: ScaleGestureDetector): Boolean {
                ...
                playerView.player?.videoSize?.let { videoSize ->
                    ...
                    val updatedVideoScale =
                        (exoContentFrameLayout.width * scaleFactor) / videoSize.width.toFloat()
                    ...
                    val currentVideoScale =
                        (exoContentFrameLayout.width * exoContentFrameLayout.scaleX) / videoSize.width.toFloat()
