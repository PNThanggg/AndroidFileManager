...
class RootHelpers(val activity: Activity) {
    ...
    fun getFiles(
        path: String, callback: (originalPath: String, listItems: ArrayList<ListItem>) -> Unit
    ) {
        getFullLines(path) {
            val fullLines = it
    ...
    private fun getChildrenCount(
        ...
        path: String,
        callback: (originalPath: String, listItems: ArrayList<ListItem>) -> Unit
    ) {
        val hiddenArgument = if (activity.config.shouldShowHidden()) "-A " else ""
        var cmd = ""
        ...
        val command = object : Command(0, cmd) {
            override fun commandCompleted(id: Int, exitcode: Int) {
                ...
                if (fileDirItems.size == 1) {
                    ...
                    copyMoveFiles(
                        fileDirItems, destination, isCopyOperation, newSuccesses, callback
                    )
                }
                super.commandCompleted(id, exitcode)
            }
