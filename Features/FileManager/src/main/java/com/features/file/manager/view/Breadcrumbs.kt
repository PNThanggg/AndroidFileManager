package com.features.file.manager.view
...
import com.features.file.manager.databinding.ItemBreadcrumbBinding
import com.features.file.manager.databinding.ItemBreadcrumbFirstBinding
...
import com.module.core.extensions.humanizePath
import com.module.core.extensions.onGlobalLayout
import com.modules.core.ui.R
...
class Breadcrumbs(
    ...
    private val textColorStateList: ColorStateList
        get() = ColorStateList(
            arrayOf(intArrayOf(android.R.attr.state_activated), intArrayOf()), intArrayOf(
                textColor, textColor.adjustAlpha(0.6f)
            )
        )
    ...
    init {
        ...
        textColor = context.resources.getColor(R.color.default_text_color, context.theme)
    }
    ...
    private fun addBreadcrumb(item: FileDirItem, index: Int, addPrefix: Boolean) {
        if (itemsLayout.childCount == 0) {
            val firstItemBgColor = if (isShownInDialog && context.baseConfig.isUsingSystemTheme) {
                ...
                resources.getColor(R.color.you_background_color, context.theme)
            }
